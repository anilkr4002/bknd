...
// API to return the latest extracted resume text
...
// API to interact with Gemini AI based on latest resume
app.post("/chat", async (req, res) => {
  ...
  try {
    // Read the latest resume dynamically
    const data = fs.readFileSync("./Resume.pdf");
    const parsed = await pdfParse(data);
    const resumeText = parsed.text;

    const model = genAI.getGenerativeModel({
      model: "gemini-1.5-flash", // or "gemini-1.5-pro" for more powerful responses
    });
